...
class Sum extends Component {
   ...
   updateNumber1(value) {
    this.setState({ number1: parseInt(value, 10) });            /*  CONVENTS STRING TO NUMBER  */
   }
   updateNumber2(value) {
    this.setState({ number2: parseInt(value, 10) });            /*  CONVENTS STRING TO NUMBER  */
   }
   add(num1, num2) {
    ...
    render() {
        return (
            <div className="puzzleBox sumPB">
                ...
                <span className="resultBox"><br/>SUM: { this.state.sum }</span>
            </div>
        )
    }
}
